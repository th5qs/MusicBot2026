...
# --- FAST WEB PORT BINDING FOR 24/7 RENDER FREE TIER ---
class WebServer(http.server.SimpleHTTPRequestHandler):
    def do_GET(self): self.send_response(200); self.send_header("Content-type", "text/plain"); self.end_headers(); self.wfile.write(b"Bot is Online 24/7!")
def run_web_server():
    with socketserver.TCPServer(("0.0.0.0", int(os.environ.get('PORT', 8080))), WebServer) as httpd: httpd.serve_forever()
...
async def on_message(message):
    ...
    parts = message.content.strip().split(maxsplit=1); if not parts: return
    cmd = parts.lower(); args = parts if len(parts) > 1 else ""; ctx = await bot.get_context(message)
