import discord, asyncio, yt_dlp, re, os, http.server, socketserver
from discord.ext import commands
from threading import Thread


# --- BUILT-IN LIGHTWEIGHT PORT BINDING FOR RENDER FREE TIER ---
...
class YTDLSource(discord.PCMVolumeTransformer):
    def __init__(self, source, *, data, user, volume=1.0):
        ...
        self.data, self.title, self.user = (
            data,
    ...
    async def from_url(cls, url, *, user):
        ...
        if "entries" in data and data["entries"]:
            data = data["entries"][0]
        return cls(
            discord.FFmpegPCMAudio(data["url"], **FFMPEG_OPTIONS), data=data, user=user
        )
...
def play_next(ctx):
    ...
    if g_id in music_queues and music_queues[g_id]:
        ...
        asyncio.run_coroutine_threadsafe(
            ctx.send(embed=create_music_embed(next_song), view=MusicControlView(ctx)),
            bot.loop,
        )
    else:
...
def create_music_embed(player):
    ...
    embed.add_field(
        name="Playing Song",
        value=f"**[{player.title}](https://youtube.com)**",
        inline=False,
    )
    embed.add_field(
        name="Song Duration", value=f"**{player.duration_str}**", inline=False
    )
    embed.set_footer(
        text=player.user.display_name, icon_url=player.user.display_avatar.url
    )
...
class MusicControlView(discord.ui.View):
    ...
    async def loop_btn(self, i: discord.Interaction, b: discord.ui.Button):
        ...
        await i.response.send_message(
            f"🔁 Loop: **{'ENABLED' if loop_status[g_id] else 'DISABLED'}**.",
            delete_after=2,
        )
    ...
    async def vol_down(self, i: discord.Interaction, b: discord.ui.Button):
        ...
        if self.ctx.guild.id in current_songs:
            current_songs[self.ctx.guild.id].volume = max(
                0.0, current_songs[self.ctx.guild.id].volume - 0.2
            )
    ...
    async def vol_up(self, i: discord.Interaction, b: discord.ui.Button):
        await i.response.defer()
        if self.ctx.guild.id in current_songs:
            current_songs[self.ctx.guild.id].volume = min(
                2.0, current_songs[self.ctx.guild.id].volume + 0.2
            )
...
async def on_voice_state_update(member, before, after):
    ...
    if (
        ...
        if manual_leave.get(guild.id, False):
            ...
            return
        if (
            guild.id in persistent_channels
            or guild.id in current_songs
            or guild.id in music_queues
        ):
    ...
    if (
        vc
        and vc.channel
        and len([m for m in vc.channel.members if not m.bot]) == 0
        and guild.id not in persistent_channels
    ):
...
async def on_message(message):
    ...
    if "setup" in cmd and bot.user.mentioned_in(message):
        if message.author.voice:
            manual_leave[message.guild.id] = False
            persistent_channels[message.guild.id] = message.author.voice.channel
            if message.guild.voice_client:
                await message.guild.voice_client.move_to(message.author.voice.channel)
            else:
                await message.author.voice.channel.connect()
            await ctx.send(
                f"🔒 **24/7 Setup Activated** in **{message.author.voice.channel.name}**."
            )
        return
    if cmd in ["p", "ش"]:
        if not args:
        ...
        try:
            ...
            if ctx.voice_client.is_playing() or ctx.voice_client.is_paused():
                ...
                await ctx.send(
                    embed=create_music_embed(player), view=MusicControlView(ctx)
                )
